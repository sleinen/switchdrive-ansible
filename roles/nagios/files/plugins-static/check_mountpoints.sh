...
# @version: 1.18
# @date: 2014-03-24 1:48:00 CEST
#
# changes 1.18
# - add support for xfs (Alessandro Usai)
# changes 1.17
...
if [ ${AUTO} -eq 1 ]; then
MPS=`${GREP} -v '^#' ${FSTAB} | awk '{if ($'${FSF}'=="nfs" || $'${FSF}'=="nfs4" || $'${FSF}'=="davfs" || $'${FSF}'=="cifs" || $'${FSF}'=="fuse" || $'${FSF}'=="glusterfs" || $'${FSF}'=="ocfs2" || $'${FSF}'=="xfs"){print $'${MF}' }}' | tr '\n' ' '`
fi
...
awk '{if ($'${FSF}'=="nfs" || $'${FSF}'=="nfs4" || $'${FSF}'=="davfs" || $'${FSF}'=="cifs" || $'${FSF}'=="fuse" || $'${FSF}'=="glusterfs" || $'${FSF}'=="ocfs2" || $'${FSF}'=="xfs"){print $'${MF}'}}' ${FSTAB} | ${GREP} -q ${MP} &>/dev/null
                if [ $? -ne 0 ]; then
log "CRIT: ${MP} doesn't exist in /etc/fstab"
        ...
        ## check kernel mounts
        ${GREP} "${MP}" ${MTAB} | ${GREP} -q -E "(nfs|nfs4|davfs|cifs|fuse|simfs|glusterfs|ocfs2|xfs)" &>/dev/null
        if [ $? -ne 0 ]; then
